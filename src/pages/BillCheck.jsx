...
export default function BillCheck() {
    ...
    async function loadBills() {
        ...
        try {
            ...
            // Transform to match existing structure expected by UI (key, vendors object for compat if needed)
            // The view returns flat structure: vendor_id, bill_no, display_date, display_bill_no, vendor_name, is_checked, uploaded_at

            const bills = data.map(b => ({
                key: `${b.vendor_id}-${b.bill_no}`,
                vendor_id: b.vendor_id,
                bill_no: b.bill_no, // Original bill no for querying lines
                purchase_date: b.display_date, // View already handles override logic
                display_date: b.display_date,
                display_bill_no: b.display_bill_no,
                is_checked: b.is_checked,
                uploaded_at: b.uploaded_at,
                vendors: { name: b.vendor_name } // Keep structure for UI compatibility
            }));
    ...
    const checkedBills = allBills.filter((b) => {
        ...
        if (searchQuery && !b.display_bill_no.toLowerCase().includes(searchQuery.toLowerCase()))
            return false;
        return true;
    });
    ...
    async function openBill(bill) {
        setLoading(true);
        try {
            // Use original bill_no and vendor_id to fetch lines
            const [linesRes, metaRes, vendorRes] = await Promise.all([
                supabase
                    .from("raw_inward")
                    .select(
                        "id, qty, rate, raw_material_id, purchase_date, raw_materials(id, name, unit, gst_rate, accounting_name)"
                    )
            ...
            setBillMeta({
                ...
                purchase_date: meta?.purchase_date || bill.purchase_date,
                bill_no_override: meta?.bill_no_override || bill.bill_no,
            });
    ...
    async function save(check = false) {
        ...
        try {
            ...
            const payload = {
                ...
                purchase_date: billMeta.purchase_date, // Using override date if set
                ...
                bill_no_override: billMeta.bill_no_override, // Saving Bill No override
            };
    ...
    async function downloadSelected() {
        ...
        try {
            ...
            for (const bill of billsToProcess) {
                ...
                // USE OVERRIDES
                const finalDate = meta.purchase_date || bill.purchase_date;
                const finalBillNo = meta.bill_no_override || bill.bill_no;
                ...
                processed.forEach((line, index) => {
                    ...
                    allRows.push({
                        ...
                        "Invoice No": finalBillNo, // Override
                        "Supplier Invoice No": finalBillNo, // Override
            ...
            <div className="card">
                <div className="hd">
                    ...
                    <div className="row" style={{ gap: 10, alignItems: "center" }}>
                        ...
                        <input
                            type="text"
                            value={billMeta.bill_no_override}
                            onChange={e => setBillMeta({ ...billMeta, bill_no_override: e.target.value })}
                    ...
                    <div className="row" style={{ gap: 5, alignItems: "center" }}>
                        ...
                        <input
                            type="date"
                            value={billMeta.purchase_date || ""}
                            onChange={e => setBillMeta({ ...billMeta, purchase_date: e.target.value })}
                        />
                    </div>
                        ...
                        <tbody>
                            {(tab === "pending" ? pendingBills : checkedBills).map((b) => (
                                <tr key={b.key}>
                                    ...
                                    <td>{b.display_date}</td>
                                    <td>{b.display_bill_no}</td>
