...
const CUSTOMER_LIST = [
  'Amazon Cons Banglore','Amazon Cons DED3','Amazon Cons Haryana','Amazon Cons Hyderabad','Amazon Cons Kolkatta','Amazon Cons Maharashtra',
  'Flipkart Cons Bhiwandi','Flipkart Cons Guwahati','Flipkart Cons Haringhata','Flipkart Cons Hyderabad','Flipkart Cons Jaipur','Flipkart Cons Kolkatta','Flipkart Cons Malur','Flipkart Cons Sankpa',
  'Jiomart Cons Haryana','Jiomart Cons Hyderabad','Jiomart Cons Kolkatta','Jiomart Cons Mumbai'
]
...
export default function Dashboard(){
  ...
  // customers (fixed to 30d)
  const [cust, setCust] = useState([])

  async function loadInitial(){
    setLoading(true)
    try {
      const { data: s, error: e1 } = await supabase.rpc('dash_sales_last_days', { p_days: 7 })
      const { data: r, error: e2 } = await supabase.rpc('dash_returns_last_days', { p_days: 7 })
      if (e1) throw e1
      if (e2) throw e2
  ...
  async function loadCustomersFixed(){
    ...
    setCust(CUSTOMER_LIST.map(name => ({
      customer_name: name,
          ...
          <div className="bd" style={{overflow:'auto', maxHeight: 360}}>
            <table className="table">
              ...
              <tbody>
                ...
                {(!top || top.length===0) && (
                  <tr><td colSpan={3} style={{color:'var(--muted)'}}>No sales</td></tr>
                )}
              </tbody>
            </table>
          </div>
  ...
  return (
    <div style={{position:'relative'}}>
      {hover && (
        <div
          style={{
            position:'absolute', left:hover.x, top:hover.y - 34,
            transform:'translate(-50%, -100%)',
            background:'var(--surface)', border:'1px solid var(--border)',
            padding:'6px 8px', borderRadius:8, fontSize:12, whiteSpace:'nowrap',
            boxShadow:'var(--shadow-1)', pointerEvents:'none', zIndex:5
          }}
        >
          <b>{hover.label}</b>
        </div>
          ...
          return (
            <div key={idx} style={{display:'grid', alignItems:'end'}}>
              <div
                ...
                style={{
                  height: Math.max(6, h),
                  background:'linear-gradient(180deg, rgba(106,167,255,.85), rgba(139,92,246,.85))',
