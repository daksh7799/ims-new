...
export default function BOM() {
  ...
  const [fgName, setFgName] = useState("");        // For display in select
  const [lines, setLines] = useState([]);          // {raw_material_id, raw_material_name, qty_per_unit, unit}
  const [loading, setLoading] = useState(false);
  const [dirty, setDirty] = useState(false);

  // NOTE: We no longer fetch ALL raw materials on mount. 
  // We use AsyncRMSelect for efficient searching.
  ...
  async function loadBOM(id) {
    ...
    const { data, error } = await supabase
      .from("v_bom_for_fg")
      .select("raw_material_id, raw_material_name, qty_per_unit, raw_material_unit")
    ...
    setLines((data || []).map(r => ({
      ...
      unit: r.raw_material_unit || ""
    })));
    setDirty(false);
  }
  useEffect(() => { loadBOM(fgId) }, [fgId]);

  function addLine() { setLines(ls => [...ls, { raw_material_id: "", raw_material_name: "", qty_per_unit: "", unit: "" }]); setDirty(true); }
  ...
  // AsyncRMSelect handles the name lookup, so we just update the line
  function onRmChange(i, newId, item) {
    updateLine(i, {
  ...
  async function saveAll() {
    ...
    setLoading(false);
    if (error) return push(error.message, "err");
    push("BOM saved successfully!", "ok");
    setDirty(false);
    setFgId(""); setFgName(""); setLines([]);
  }
  ...
  // Bulk upload still needs all masters to validate names. 
  // We load them only when the user switches to the "Bulk" tab or clicks upload.
  // For now, we'll keep the effect but only run if tab is 'bulk'
  useEffect(() => {
    if (tab !== 'bulk') return;
    if (fgs.length > 0 && rmsAll.length > 0) return; // already loaded
  ...
  function onFile(e) {
    ...
    reader.onload = ev => {
      ...
      } catch (err) { push("File read failed: " + err.message, "err"); setRows([]); setErrors([]); setResult(null); }
  ...
  async function upload() {
    ...
    try {
      ...
      if (missing.length) { push('Name mismatches:\n' + [...new Set(missing)].slice(0, 30).join('\n'), "err"); setBulkLoading(false); return; }
      ...
      push(`Successfully updated ${summary.length} BOMs`, "ok");
      setRows([]); setErrors([]);
    } catch (err) {
      console.error(err);
      setResult({ ok: false, msg: err.message });
      push("Upload failed: " + err.message, "err");
    } finally {
          ...
          {tab === "manual" && (
            <>
                      ...
                      <tr key={idx}>
                        <td>
                          <AsyncRMSelect
                            ...
                            preselectedName={l.raw_material_name}
                            onChange={(id, item) => onRmChange(idx, id, item)}
                          />
                        </td>
              ...
              <div className="row" style={{ marginTop: 10 }}>
                <button className="btn outline" onClick={addLine} disabled={!fgId}>+ Add RM</button>
